...
class LongformerEmbeddings(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.token_type_embeddings = nn.Embedding(config.type_vocab_size, config.model_dim)

        # self.LayerNorm is not snake-cased to stick with TensorFlow model variable name and be able to load
        # any TensorFlow checkpoint file
        self.LayerNorm = nn.LayerNorm(config.model_dim, eps=config.layer_norm_eps)
        self.dropout = nn.Dropout(config.hidden_dropout_prob)

        self.padding_idx = config.pad_token_id
        self.position_embeddings = nn.Embedding(
            config.max_position_embeddings, config.model_dim, padding_idx=self.padding_idx
        )
...
class LongformerSelfAttention(nn.Module):
    def __init__(self, config, layer_id):
        ...
        if config.model_dim % config.num_attention_heads != 0:
            raise ValueError(
                f"The hidden size ({config.model_dim}) is not a multiple of the number of attention "
                f"heads ({config.num_attention_heads})"
            )
        self.num_heads = config.num_attention_heads
        self.head_dim = int(config.model_dim / config.num_attention_heads)
        self.embed_dim = config.model_dim

        self.query = nn.Linear(config.model_dim, self.embed_dim)
        self.key = nn.Linear(config.model_dim, self.embed_dim)
        self.value = nn.Linear(config.model_dim, self.embed_dim)

        # separate projection layers for tokens with global attention
        self.query_global = nn.Linear(config.model_dim, self.embed_dim)
        self.key_global = nn.Linear(config.model_dim, self.embed_dim)
        self.value_global = nn.Linear(config.model_dim, self.embed_dim)
...
class LongformerModel(nn.Module):
    ...
    attention are more relevant for autoregressive language modeling than finetuning on downstream tasks.
    """
